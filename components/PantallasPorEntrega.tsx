						...
						<div className="p-3">
							<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
								{pantallasGrupo.map((pantalla) => (
									<div
										key={pantalla.id}
										className={`p-2 rounded border transition-all hover:shadow-sm relative ${getColorEstado(
											pantalla,
										)}`}
										title={pantalla.nombre}
									>
										{/* Número de prioridad en esquina superior derecha */}
										{pantalla.prioridadNum && (
											<div className="absolute -top-1.5 -right-1.5 bg-indigo-600/80 text-white text-xs font-bold rounded-full w-6 h-6 p-3 flex items-center justify-center shadow-md border-2 border-white">
												{pantalla.prioridadNum}
											</div>
										)}
										...
										<div className="flex gap-1.5 mt-1">
											{pantalla.revisionEstetica && !pantalla.revisionEsteticaGrupal && (
												<span className="text-xs">🎨</span>
											)}
											{pantalla.revisionFluidez && !pantalla.revisionFluidezGrupal && (
												<span className="text-xs">⚡</span>
											)}
											{pantalla.revisionEsteticaGrupal && (
												<span className="text-xs ring-green-800/40 ring-2 rounded px-1">
													🎨✓
												</span>
											)}
											{pantalla.revisionFluidezGrupal && (
												<span className="text-xs ring-green-800/40 ring-2 rounded px-1">
													⚡✓
												</span>
											)}
