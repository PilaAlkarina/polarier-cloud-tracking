...
export function getColorEstado(pantalla: Pantalla): string {
	if (pantalla.estado.includes("Completada")) {
		if (pantalla.revisionEsteticaGrupal && pantalla.revisionFluidezGrupal) {
			return "bg-green-50 text-green-700 border-green-500";
		}
		if (pantalla.revisionEsteticaGrupal || pantalla.revisionFluidezGrupal) {
			return "bg-yellow-50 text-yellow-700 border-yellow-500";
		}
		if (pantalla.revisionEstetica || pantalla.revisionFluidez) {
			return "bg-orange-50 text-orange-700 border-orange-500";
		}
		return "bg-red-50 text-red-700 border-red-500";
	}
	if (pantalla.estado.includes("Verificar")) return "bg-blue-50 text-blue-700 border-blue-500";
	if (pantalla.estado.includes("Bloqueada")) return "bg-red-50 text-red-700 border-red-500";
	if (pantalla.estado.includes("estética")) return "bg-purple-50 text-purple-700 border-purple-500";
	if (pantalla.estado.includes("fluidez")) return "bg-indigo-50 text-indigo-700 border-indigo-500";
